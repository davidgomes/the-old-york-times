...
Meteor.methods({
  ...
  'News.methods.getNews'({ startYear, endYear, region }) {
    ...
    if (region == "World") {
      newsList = News.find({ date: { $gt: startYear, $lt: endYear } }, { sort: { score: -1 }, limit: 120 }).fetch();
    } else if (_.contains(regionArray, region)) {
      console.log("Searching for " + region);
      newsList = News.find({ date: { $gt: startYear, $lt: endYear }, region: region }, { sort: { score : -1 }, limit: 120 }).fetch();
    } else {
      const country = region.toLowerCase();
      newsList = News.find({ date: { $gt: startYear, $lt: endYear }, country: country }, { sort: { score : -1 }, limit: 120 }).fetch();
    }
  ...
  'News.methods.starImage'({ headline }) {
    ...
    News.find({ headline: headline }, { $inc: score });
